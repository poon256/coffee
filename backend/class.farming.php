    ...
    function def()
    {
        ...
        <div class='container'>
            <div class='row'>
				...
				if ($acl > '3') {
				?>
                <input type='button' value='print' onclick='window.open("print.php?cat=farming&typ=all","_self")'>
                <?php
				}
				?>
                ...
                <table id='datatable' class='table table-bordered table-striped'>
                    ...
                    <tbody>
                        <?php
                        $a = 1;
                        $sql = "select `batch`.`id` as `id`, `farming`.`status` as `status`, `farming`.`date` as `date`, (select `users`.`name` from `users` where `users`.`status` = '1' and `users`.`id` = `farming`.`uaid`) as `uaid` from `farming`, `batch`
                        where `farming`.`batch_id` = `batch`.`id`";
                        ...
                        while ($cdr = $res->fetch())
                        {
                            ...
                            echo "<input type='button' value='".$ds."' onclick='confirm_del(\"index.php?option=farming&task=del&id=".$cdr['id']."&stat=".$dss."\")' />";
							}
							if ($cdr['status'] == 1)
                            {
                                echo "<input type='button' value='Approve' onclick='window.open(\"index.php?option=farming&task=del&id=".$cdr['id']."&stat=2\",\"_self\")' />";
							}
                            echo "</td>";
                            }
                            echo "</tr>";
            ...
            <div class='row'>
                ...
                <table id='datatable' class='table table-bordered table-striped'>
                    ...
                    <tbody>
                        ...
                        where batch_detail.id = batch.id and `batch`.`status` = '2' and `batch`.`id` not in (select `farming`.`batch_id` from `farming` where `farming`.`status` > 0)";
						//echo $sql;
                        $conn = new connect();
                        $res = $conn->query($sql);
                        while ($cdr = $res->fetch())
                        {
                            ...
                            echo "<input type='button' value='Edit' onclick='window.open(\"index.php?option=farming&task=edit&id=0&ref=".$cdr['id']."\",\"_self\")' />";  
                            echo "</td>";
                            echo "</tr>";
                            $a++;
                        }
...
else
{
    ...
    function del()
    {
		...
		if ($stat == 2)
		{
			$sql = "update farming set status = '".$stat."', `uaid` = '".$_SESSION['uid']."' where id = '".$id."' ";
            $conn->save_logs("Appove farming#".$id, $_SESSION['uid']);
		}
		else
		{
            ...
            if ($stat == 1)
            {
                $conn->save_logs("Active farming#".$id, $_SESSION['uid']);
            }
            elseif ($stat == 0)
            {
                $conn->save_logs("In-Active farming#".$id, $_SESSION['uid']);
            }
		}
        $conn->query($sql);
        header("location:index.php?option=farming&task=def");
        ...
        <div class='container'>
            <div class='row'>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    <tr>
                        <td colspan='2' class='text-center'>
                            <input type="button" value="Print" onclick="window.open('print.php?cat=farming&typ=det&id=<?php echo $id;?>','_self')">
                            <input type="button" value="Back" onclick="window.open('index.php?option=farming&task=def','_self')">
                        </td>
                    </tr>
                </table>
